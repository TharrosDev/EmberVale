...
def gates(engine: str | None) -> list[Gate]:
    ...
    return [
        ...
        Gate("environment-climate", "prepared climate retains warm rain, alpine snow and legacy compatibility",
             [engine, "--headless", "--path", ".", "--script",
              "res://tools/environment_climate_probe.gd"], modes=("engine", "full")),
        Gate("environment-route", "actual gameplay renders day, weather, quality tiers and sheltered interiors",
             [sys.executable, "tools/embervale.py", "tool", "environment_route", "--render", "--timeout", "300"],
             modes=("visual", "performance", "full"), timeout=360),
        ...
        Gate("regressions", "the 2026-08-30 debugging pass's defects stay fixed",
             ...
             expected_errors=(r"Cannot open file 'res://data/world_bake/cells/region_ember_crown/ember_crown___does_not_exist.scn'",
                              r"Failed loading resource: res://data/world_bake/cells/region_ember_crown/ember_crown___does_not_exist.scn",
                              r"Error loading resource: 'res://data/world_bake/cells/region_ember_crown/ember_crown___does_not_exist.scn'",
                              r"RegionStreamer: cell 'ember_crown\.__does_not_exist' (headless scene load failed|threaded load failed \(Failed\)); giving up after 3 attempts")),
        ...
        Gate("action-clip", "on a rigged body the clip IS the clock, warped or natural",
             # Compare animation idle time to physics ticks on a fixed engine clock; retain
             # the probe's real rig, hit window and existing duration tolerances.
             [engine, "--headless", "--path", ".", "--fixed-fps", "60", "--script", "res://tools/action_clip_probe.gd"],
             slow=True),
