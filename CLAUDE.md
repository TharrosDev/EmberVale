...
`LMB` attack · `RMB` block · `H` heal dummy · `R` respawn dummy · `F5`/`F9`
quick save/load · `Esc` pause (frees the cursor). Goblins roam to the north (−Z).
...
- **`GameBootstrap : Node3D`** (`scenes/Main.tscn` root) — assembles the sandbox:
  registers input, builds the world (directional light, sky, floor mesh +
  `WorldBoundaryShape3D` collider), the `DebugHud`, the training dummy (team 2),
  the player, and the goblin camp. Runs with `ProcessMode.Always` so it can
  unpause and read debug keys while paused. Routes player/dummy/enemy deaths.
...
4 Enemy AI**. Next: **5 Inventory System**.

Then (in order): 6 Equipment · 7 Loot Generation · 8 Progression · 9 Quests ·
10 Dialogue · 11 NPC Schedules · 12 Magic · 13 World Systems · 14 Crafting ·
15 Factions · 16 Procedural Events · 17 Optimization · 18 Content Expansion.
