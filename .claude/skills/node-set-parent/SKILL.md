...
```bash
curl -X POST http://localhost:23630/api/tools/node-set-parent \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/node-set-parent -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
