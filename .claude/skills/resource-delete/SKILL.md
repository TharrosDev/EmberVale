...
```bash
curl -X POST http://localhost:23630/api/tools/resource-delete \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/resource-delete -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
