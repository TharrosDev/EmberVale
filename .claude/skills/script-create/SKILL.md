...
```bash
curl -X POST http://localhost:23630/api/tools/script-create \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/script-create -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
