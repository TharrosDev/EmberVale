...
```bash
curl -X POST http://localhost:23630/api/tools/runtime-errors-clear \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/runtime-errors-clear -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
