...
```bash
curl -X POST http://localhost:23630/api/tools/resource-create \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/resource-create -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
