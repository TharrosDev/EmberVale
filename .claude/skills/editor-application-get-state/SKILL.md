...
```bash
curl -X POST http://localhost:23630/api/tools/editor-application-get-state \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/editor-application-get-state -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
