...
```bash
curl -X POST http://localhost:23630/api/tools/resource-get-data \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:23630/api/tools/resource-get-data -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
