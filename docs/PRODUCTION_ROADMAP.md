...
- **39.5B ✅ — the player HUD** (maintainer direction, 2026-08-11: briefed as a standalone HUD
  ...
  a distance-and-bearing readout, one `--validate` arm, two negative tests and 38 unit tests. ⚠️ **A
  death HUD and a combat HUD state were CUT and named** — respawn is synchronous so there is no
  window for one, and there is no `InCombat` flag for the other to read. The section-by-section audit
  is in [`docs/playbook/phase-39_5.md`](playbook/phase-39_5.md).
...
| 39.5 | ⏳ | **World map & location intelligence — 39.5A and 39.5B closed.** The map reads authoritative world data instead of the three things that happened to have coordinates. ⚠️ **A location's position is its node's transform in a cell scene, never an authored coordinate**, and `--validate` checks that seam in both directions. Author with `tools/gen_map_locations.py`. **39.5B was the player HUD**: a minimap (none existed), one tracked-quest authority replacing two independent first-active scans, and the visibility logic `GameHud` had never had. ⚠️ **Its audit found ~60 of the brief's 80 sections already satisfied** — the finding was that the HUD was mature, not broken. **39.5C is the live sub-phase** and carries a runnable condition per deferred item — see [`docs/playbook/phase-39_5.md`](playbook/phase-39_5.md). ⚠️ Its first item is the harness gap rather than a feature: **nothing here can see a UI screen, and that has now cost two sub-phases** — three screen-space defects in 39.5A, and a whole HUD shipped unrendered in 39.5B |
