...
- **39.5B ⏳ — THE LIVE ITEM** (maintainer direction, 2026-08-10). Cartography, quests on the map,
  districts, clustering, live NPC positions, routes — plus the UI-render harness gap 39.5A exposed.
...
| 39.5 | ⏳ | **World map & location intelligence — 39.5A closed.** The map reads authoritative world data instead of the three things that happened to have coordinates. ⚠️ **A location's position is its node's transform in a cell scene, never an authored coordinate**, and `--validate` checks that seam in both directions. Author with `tools/gen_map_locations.py`. **39.5B is the live sub-phase** and carries a runnable condition per deferred item — see [`docs/playbook/phase-39_5.md`](playbook/phase-39_5.md). ⚠️ Its first item is the harness gap rather than a feature: three screen-space defects shipped through a fully green battery because nothing here can see a UI screen |
