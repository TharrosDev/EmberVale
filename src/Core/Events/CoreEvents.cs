...
/// <summary>Raised when an <see cref="Entity"/> enters the world.</summary>
public readonly record struct EntitySpawnedEvent(Entity Entity) : IGameEvent;

/// <summary>Raised when an <see cref="Entity"/> is about to leave the world.</summary>
public readonly record struct EntityDespawnedEvent(Entity Entity) : IGameEvent;

/// <summary>Raised when an entity's health reaches zero.</summary>
public readonly record struct EntityDiedEvent(Entity Entity) : IGameEvent;

/// <summary>Raised when an entity takes damage. <paramref name="RemainingHealth"/> is post-damage.</summary>
public readonly record struct EntityDamagedEvent(Entity Entity, float Amount, float RemainingHealth) : IGameEvent;

/// <summary>Raised when an entity is healed. <paramref name="CurrentHealth"/> is post-heal.</summary>
public readonly record struct EntityHealedEvent(Entity Entity, float Amount, float CurrentHealth) : IGameEvent;

/// <summary>Raised when a current/max resource (health, stamina, mana) changes value.</summary>
public readonly record struct ResourceChangedEvent(Entity Entity, StatType Stat, float Current, float Max) : IGameEvent;
