...
press that the paused game would have ignored. The rest of it got a proper pass too: your health is
now the biggest thing in the corner instead of one of three identical rows, it warns you when it is
running out, the empty bars are actually visible behind the full part, the clock has a sun and a
moon, the compass fades into the horizon instead of stopping at two hard edges, and the quick-use bar
no longer writes the word EMPTY four times across the bottom of your screen.
